...
def validation(
    criterion, ap, model, c, testloader, tensorboard, step, cuda, loss1_weight=1
):
    model.zero_grad()
    model.eval()
    loss = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--seed", type=int, default=None, help="Seed for training"
    )
    ...
    if args.seed is None:
        ...
        log_path = os.path.join(
            os.path.join(c.train_config["logs_path"],
            str(args.seed)), c.model_name
        )
    ...
    trainloader = train_dataloader(
        copy_config_dict(c), ap, 
        class_balancer_batch=c.dataset["class_balancer_batch"]
    )
